...
var LASERTRACKER = (function () {
  ...
  function parseTime (t) {
    ...
    var multiplier = 60
    var seconds = t.split(':').reduceRight(function (prev, cur) {
  ...
  function appendLedger (snapshot) {
    ...
    $('#ledger .header').after(
      $('<tr>').append(
        ...
        (firstCol = $('<td>').text(
          new Date(row.date).toString('MMM d, yyyy h:mm tt')
        )),
        $('<td>').text(row.name),
        $('<td>').text(row.project),
        $('<td>').text(row.time),
  ...
  function undoLedger (row) {
    ...
    if (currentUser && currentUser.isAdmin) {
      if (
        confirm(
          'Do you want to reverse charges for "' +
            row.project +
            '" for ' +
  ...
  function validateTrack () {
    ...
    ;['#name', '#project', '#time'].forEach(function (id) {
      $(id)
        .parent()
        .toggleClass('has-error', !$(id).val())

      if (!$(id).val()) result = false
    })
  ...
  exports.init = function init () {
    ...
    firebase
      .child('ledger')
      .limitToLast(100)
      .on('child_added', appendLedger)
    firebase
      .child('totals')
      .child('paid')
      .on('value', updateTotalPaid)
    firebase
      .child('totals')
      .child('time')
      .on('value', updateTotalTime)
  }
  ...
  exports.rebuildTotals = function rebuildTotals () {
    ...
    var commit = function () {
      ...
      $('#adminButton')
        .popover({ content: 'Done!', placement: 'bottom' })
        .popover('show')
    }
